...
import { Eye, EyeOff, Loader2, Heart, Activity, Shield } from "lucide-react";
    ...
    <div className="min-h-screen flex" style={{background:"#f0f9ff"}}>
      ...
      <motion.div initial={{opacity:0,x:-20}} animate={{opacity:1,x:0}} transition={{duration:.5}}
        ...
        <div className="max-w-md w-full mx-auto">
          ...
          <div className="mb-8">
            <h1 className="text-3xl font-bold mb-2" style={{color:"#0f172a",fontFamily:"Plus Jakarta Sans,sans-serif"}}>Welcome back</h1>
            <p className="text-sm" style={{color:"#64748b"}}>Sign in to your health intelligence dashboard</p>
          </div>
          ...
          <form onSubmit={handleSubmit} className="space-y-4">
            ...
            <div>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <Link href="/forgot-password" className="text-xs font-semibold" style={{color:"#0284c7"}}>Forgot password?</Link>
              </div>
            ...
            <button type="submit" disabled={loading} className="btn btn-primary w-full py-3 text-sm">
              {loading ? <><Loader2 className="w-4 h-4 animate-spin"/>Signing in…</> : "Sign in to Helixa"}
            </button>
          </form>

          <p className="text-center text-sm mt-6" style={{color:"#64748b"}}>
            New to Helixa?{" "}
            <Link href="/signup" className="font-semibold" style={{color:"#0284c7"}}>Create an account</Link>
          </p>
        </div>
      </motion.div>
      ...
      <motion.div initial={{opacity:0}} animate={{opacity:1}} transition={{duration:.7,delay:.2}}
        ...
        style={{background:"linear-gradient(155deg, #0284c7 0%, #0d9488 100%)"}}>
        ...
        <div>
          <h2 className="text-3xl font-bold text-white mb-4" style={{fontFamily:"Plus Jakarta Sans,sans-serif"}}>
            Your complete health record, powered by AI
          </h2>
          <p className="text-sm leading-relaxed mb-8" style={{color:"rgba(255,255,255,.65)"}}>
            Vitals, medications, documents, AI insights, doctor discovery, and FDA drug data — all in one secure, interconnected platform.
          </p>

          <div className="space-y-3.5">
            {[
              { icon:Heart,    label:"Longitudinal health graph from birth" },
              { icon:Activity, label:"AI-analyzed vitals & trend detection" },
              { icon:Shield,   label:"Explainable AI with doctor review" },
            ].map(({ icon:Icon, label }) => (
              <div key={label} className="flex items-center gap-3">
                ...
                <span className="text-sm" style={{color:"rgba(255,255,255,.85)"}}>{label}</span>
              </div>
            ))}
          </div>
        </div>
